...
const Main = () => {
  ...
  //   console.log('ff', data[currentquestion].question)
  ...
  // let star = <i class="fa-solid fa-star"/>
  ...
  // progress bar, score bar, maxsvore bar
  let progress = `${(currentquestion / (data.length-1)) * 100}%`;
  let scores = `${score}%`;
  let maxscores = `${maxscore}%`;


  return (
    <div className="app">
      <div className="Bar">
        <div style={{ width: progress }} className="subbar"></div>
      </div>
      ...
      <div className="Header">
        ...
        <p>{data[currentquestion].category}</p>
        <div className="stars">
          {displayStars()}
        </div>
        <div className="">
      ...
      <div className="Body">
        <div className="Question">
          <h3 className="qqq"> {data[currentquestion].question} </h3>
          <div className="answer">
            ...
            <button onClick={() => selectAnswer()} className="ans">
              {" "}
        ...
        <div className="mid-result">
          <h2> {result === true ? "Correct!" : "Sorry!"} </h2>
          <h2> {result} </h2>
          {result === true ? ( currentquestion === 19 ? "Test Completed ":
            <button onClick={() => moveToNextQuestion()} className="ans">
              Next question
            </button>
          ) : (
            " "
      ...
      <div className="scores">
        <h3>score : {scores}</h3>
        <h3>max score: {maxscores}</h3>
      </div>
      <div className="bottom">
        {/* <div style={{ width: maxscore }} className="maxscore"></div>
        <div style={{ width: score }} className="subbottom"></div> */}
      </div>
    </div>
  );
};
